...
contract testStarter is Test {
    ...
    address public owner3 = address(0x1C66eAE2BAc8b2CCeAd447a1C1FB40Eb86b7d820);
    ...
    function deployChainlinkOracle() public {
        chainlinkOracle = new ChainlinkOracleConnector(
            address(registry),
            address(0),
            0
        );
        ...
        console.log(
            "ChainlinkOracleConnector deployed: %s",
    ...
    function deploySwapHandler() public {
        swapHandler = new SwapAndBridgeHandler(
            new address[](0),
            address(noyaOracle),
            registry
        );

        lifiImplementation = new LifiImplementation(
            address(swapHandler),
            lifiDiamond
        );
        ...
        console.log(
            "LifiImplementation deployed: %s",
            address(lifiImplementation)
        );
    ...
    function addTokenToChainlinkOracle(
        address token,
        address base,
        address source
    ) public {
    ...
    function addRoutesToNoyaOracle(
        address token1,
        address token2,
        address intermidiate
    ) public {
        address[] memory assets = new address[](1);
    ...
    function addTrustedTokens(
        ...
        tokens[0] = token;
        registry.updateConnectorTrustedTokens(
            _vaultId,
            connector,
            tokens,
            true
    ...
    function addConnectorToRegistry(
        uint256 _vaultId,
        address connector
    ) public {
