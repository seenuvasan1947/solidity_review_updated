...
contract TestOmnichain is testStarter, OptimismAddresses {
    ...
    function testGenerateLZPeerData() public {
        bytes memory data = abi.encodeCall(
            lzHelperSender.setPeer,
            (30_145, bytes32(uint256(uint160(address(0x09c740CEa5876fBCd7790e1792c32F395a50D789)))))
        );
        console.logBytes(data);

        bytes memory data2 = abi.encodeCall(lzHelperSender.updateMessageSetting, (createLzReceiveOption(1e6, 0)));
    ...
    function testMessaging() public {
        ...
        console.log("----------- accountinmanager tvl ----------- %s", accountingManager.totalAssets());
        assertEq(accountingManager.totalAssets(), _amount, "TVL should be _amount");
        vm.stopPrank();
    }
