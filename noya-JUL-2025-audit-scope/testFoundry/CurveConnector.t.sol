...
contract TestCurveConnector is testStarter, MainnetAddresses {
    CurveConnectorMainnet connector;
    ...
    function setUp() public {
        ...
        connector = new CurveConnectorMainnet(
        ...
        addTrustedTokens(vaultId, address(accountingManager), GEAR);
        ...
        addTokenToChainlinkOracle(address(GEAR), address(840), address(CRVUSD_USD_FEED));
        addTokenToNoyaOracle(address(GEAR), address(chainlinkOracle));
        ...
        addRoutesToNoyaOracle(address(GEAR), address(USDC), address(840));
        ...
        registry.addTrustedPosition(0, 0, address(accountingManager), false, false, abi.encode(GEAR), "");
        // registry.addTrustedPosition(0, 0, address(accountingManager), false, false, abi.encode(CRV), "");
        // registry.addTrustedPosition(0, 0, address(accountingManager), false, false, abi.encode(CVX), "");
        ...
        address[] memory tokensGear = new address[](2);
        tokensGear[0] = address(GEAR);
        tokensGear[1] = address(WETH);

        PoolInfo memory poolGear = PoolInfo(
            curveGearEthPool,
            2,
            gearEthLpToken,
            gearEthCurveGauge,
            gearEthLpToken, //convex LP
            convexGearEthRewardBooster,
            address(0),
            address(0),
            tokensGear,
            address(0),
    ...
    function test_CurveConvex_extraRewards() public {
        uint256 _amount = 100_000_000;

        _dealWhale(baseToken, address(connector), USDC_Whale, 4 * _amount);
        _dealERC20(address(GEAR), address(connector), 3 * _amount);
        _dealERC20(address(WETH), address(connector), 3 * _amount);

        vm.startPrank(address(owner));

        connector.openCurvePosition(curveGearEthPool, 0, _amount, 0);
        uint256 lpBalance = IERC20(gearEthLpToken).balanceOf(address(connector));

        connector.depositIntoConvexBooster(curveGearEthPool, 136, lpBalance, true);
        address[] memory tokens = new address[](1);
        tokens[0] = convexGearEthRewardBooster;

        vm.warp(block.timestamp + 120 minutes);

        uint256 extraRewardBalanceBefore = IERC20(GEAR).balanceOf(address(connector));
        
        connector.harvestConvexRewards(tokens);
        
        uint256 extraRewardBalanceAfter = IERC20(GEAR).balanceOf(address(connector));
        assertTrue(extraRewardBalanceBefore < extraRewardBalanceAfter);
