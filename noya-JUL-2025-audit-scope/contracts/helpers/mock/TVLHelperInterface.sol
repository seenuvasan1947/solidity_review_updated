...
contract TVLHelperInterface {
    ...
    function getTVL(uint256 vaultId, address registry, address baseToken) public view returns (uint256) {
        uint256 totalTVL;
        uint256 totalDebt;
        HoldingPI[] memory positions = PositionRegistry(registry).getHoldingPositions(vaultId);
        for (uint256 i = 0; i < positions.length; i++) {
            ...
            bool isPositionDebt = PositionRegistry(registry).isPositionDebt(vaultId, positions[i].positionId);
            if (isPositionDebt) {
                totalDebt += tvl;
            } else {
                totalTVL += tvl;
    ...
    function getLatestUpdateTime(uint256 vaultId, address registry) public view returns (uint256) {
