...
abstract contract OmnichainLogic is BaseConnector {
    ...
    uint256 public bridgeWaitingTime = 30 minutes;
    ...
    function updateBridgeTransactionApproval(bytes32 transactionHash) public onlyManagerOrWatcher {
     ...
     */
    function startBridgeTransaction(BridgeRequest memory bridgeRequest) payable virtual public onlyManager nonReentrant {
       ...
       if (approvedBridgeTXN[txn] == 0 || approvedBridgeTXN[txn] + bridgeWaitingTime > block.timestamp) {
        ...
        swapHandler.executeBridge{value: msg.value}(bridgeRequest);
        _updateTokenInRegistry(bridgeRequest.inputToken);
    }

    function updateBridgeWaitingTime(uint256 newWaitingTime) external onlyMaintainer {
        require(newWaitingTime > 0, "Invalid waiting time");
        bridgeWaitingTime = newWaitingTime;
    }
}
