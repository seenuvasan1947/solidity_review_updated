...
contract SwapAndBridgeHandler is ISwapAndBridgeHandler, ReentrancyGuard {
    ...
    modifier onlyMaintainerOrEmergency() {
        if (!registry.hasRole(registry.MAINTAINER_ROLE(), msg.sender) && !registry.hasRole(registry.EMERGENCY_ROLE(), msg.sender))
            revert("NoyaSwapHandler: Unauthorized access");
        _;
    }

    constructor(address[] memory usersAddresses, address _valueOracle, PositionRegistry _registry)
    {
        for (uint256 i = 0; i < usersAddresses.length; i++) {
            isEligibleToUse[usersAddresses[i]] = true;
        }
        registry = _registry;
        require(address(_registry) != address(0), "NoyaSwapHandler: Invalid registry address");
        ...
        returns (uint256 _amountOut)
    {
        ...
        _amountOut = ISwapAndBridgeImplementation(swapImplInfo.route).performSwapAction{value: msg.value}(msg.sender, _swapRequest);
        ...
        nonReentrant
    {
        ...
        ISwapAndBridgeImplementation(bridgeImplInfo.route).performBridgeAction{value: msg.value}(msg.sender, _bridgeRequest);
     ...
     */
    function addRoutes(RouteData[] memory _routes) public onlyMaintainerOrEmergency {
