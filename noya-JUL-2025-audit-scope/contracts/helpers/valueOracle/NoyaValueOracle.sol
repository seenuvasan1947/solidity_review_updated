...
contract NoyaValueOracle is INoyaValueOracle {
    ...
    event UpdatedDefaultPriceSource(
        address[] baseCurrencies,
        INoyaValueOracle[] oracles
    );
    event UpdatedAssetPriceSource(
        address[] asset,
        address[] baseToken,
        address[] oracle
    );
    ...
    modifier onlyMaintainer() {
        if (!registry.hasRole(registry.MAINTAINER_ROLE(), msg.sender))
            revert INoyaValueOracle_Unauthorized(msg.sender);
        _;
    }
    ...
    function updateDefaultPriceSource(
        address[] calldata baseCurrencies,
        INoyaValueOracle[] calldata oracles
    ...
    function updateAssetPriceSource(
        address[] calldata asset,
        address[] calldata baseToken,
        address[] calldata oracle
