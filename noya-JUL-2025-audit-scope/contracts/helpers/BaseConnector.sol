...
contract BaseConnector is NoyaGovernanceBase, IConnector, ReentrancyGuard {
    ...
    uint256 public BASIC_POINT_DIVISOR = 10_000;

    uint256 public slippageTolerance = 2_000_000; // 0.5%

    constructor(
        BaseConnectorCP memory params
     ...
     */
    function updateMinimumHealthFactor(
    ...
    /**
     * @notice Updates Slippage Tolerance value
     * @param _slippageTolerance The new slippageTolerance value
     * @dev slippage control is used to prvent price-manipulation attacks
     */
     ...
     */
    function updateSwapHandler(
        address payable _swapHandler
    ) external onlyMaintainer {
    ...
    function sendTokensToTrustedAddress(
        ...
        if (msg.sender == accountingManager) {
            (, , , , address watcherContract, ) = registry
                .getGovernanceAddresses(vaultId);
            ...
            registry.isAnActiveConnector(vaultId, msg.sender) ||
            msg.sender == registry.flashLoan()
        ) {
            ...
            if (caller != address(this))
                revert IConnector_InvalidAddress(caller);
    ...
    function _updateTokenInRegistry(address token, bool remove) internal {
        ...
        uint256 positionIndex = registry.getHoldingPositionIndex(
            vaultId,
            positionId,
            address(this),
    ...
    function updateTokenInRegistry(address token) public onlyManager whenNotPaused {
        _updateTokenInRegistry(token);
    }
    ...
    function _updateTokenInRegistry(address token) internal {
        _updateTokenInRegistry(
            token,
            IERC20(token).balanceOf(address(this)) <= registry.dustLevel(token)
        );
    }
    /**
    ...
    function addLiquidity(
        ...
        for (uint256 i = 0; i < tokens.length; i++) {
            ...
            ITokenTransferCallBack(msg.sender).sendTokensToTrustedAddress(
                tokens[i],
                amounts[i],
                msg.sender,
                ""
            ...
            if (_balanceAfter < amounts[i] + _balance) {
                revert IConnector_InsufficientDepositAmount(
    ...
    function _getPositionTVL(
        HoldingPI memory,
        address
    ) public view virtual returns (uint256 tvl) {
        return 0;
    }
