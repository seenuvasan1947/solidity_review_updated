...
contract PositionRegistry is AccessControl, IPositionRegistry, ReentrancyGuard {
    ...
    modifier whenNotPaused(uint256 _vaultId) {
        if (vaults[_vaultId].isPaused) revert Paused();
        _;
    }

    modifier whenPaused(uint256 _vaultId) {
        if (!vaults[_vaultId].isPaused) revert NotPaused();
        _;
    }
    ...
    */
    function updateHoldingPosition(
        ...
        emit HoldingPositionUpdated(vaultId, _positionId, d, AD, false, index, msg.sender);
    ...
    */
    function updateHoldingPosition(
        ...
        bool removePosition
    ) public vaultExists(vaultId) whenNotPaused(vaultId) returns (uint256) {
        Vault storage vault = vaults[vaultId];
        ...
        if (removePosition) {
            ...
            emit HoldingPositionUpdated(vaultId, _positionId, _data, additionalData, removePosition, positionIndex, msg.sender);
    ...
    function pauseVault(uint256 vaultId) external  vaultExists(vaultId) whenNotPaused(vaultId) {
        if (msg.sender != vaults[vaultId].watcherContract && hasRole(EMERGENCY_ROLE, msg.sender) == false) {
            revert UnauthorizedAccess();
        }
    ...
    function unpauseVault(uint256 vaultId) external onlyVaultGoverner(vaultId) vaultExists(vaultId) whenPaused(vaultId) {
    ...
    */
    ...
    function isVaultPaused(uint256 vaultId) public view returns (bool) {
        return vaults[vaultId].isPaused;
    }
}
