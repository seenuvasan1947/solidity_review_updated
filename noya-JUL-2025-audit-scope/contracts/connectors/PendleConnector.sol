...
import {MarketApproxPtInLib, MarketApproxPtOutLib, MarketMathCore} from "../external/libraries/Pendle/MarketApproxPtInLib.sol";
...
contract PendleConnector is BaseConnector {
    ...
    error InsufficientLpOut(uint256 netLpOut, uint256 minAmount);
    error InsufficientPtOut(uint256 netPtOut, uint256 minAmount);
     ...
     * @param minAmount Minimum amount of shares minted
    ...
    function supply(
        ...
        (IPStandardizedYield _SY, IPPrincipalToken _PT, ) = IPMarket(market)
            .readTokens();
        ...
        uint256 syMinted = _SY.deposit(
            address(this),
            _underlyingToken,
            amount,
            minAmount
        );
        _updateTokenInRegistry(_underlyingToken);
     ...
     * @param SYamountMax Amount of SY tokens desired to deposit
     * @param PTamountMax Amount of PT tokens desired to deposit
    ...
    function depositIntoMarket(
        ...
        MarketState memory currentMarketState = IPMarket(market).readState(
            address(0)
        );
        (, , uint256 syUsed, uint256 ptUsed) = MarketMathCore.addLiquidity(
            currentMarketState,
            SYamountMax,
            PTamountMax,
            block.timestamp
        ...
        IERC20(address(_SY)).safeTransfer(address(market), syUsed);
        IERC20(address(_PT)).safeTransfer(address(market), ptUsed);

        (uint256 netLpOut, uint256 netSyUsed, uint256 netPtUsed) = market.mint(
            address(this),
            syUsed,
            ptUsed
        );
        if (netLpOut < minAmount) {
            revert InsufficientLpOut(netLpOut, minAmount);
        }

        emit DepositIntoMarket(address(market), netSyUsed, netPtUsed);
    }
    ...
    function swapYTForSY(
        ...
        uint256 exactYTIn,
        uint256 min,
        LimitOrderData memory orderData
    ...
    function swapExactPTForSY(
        ...
        (uint256 netSyOut, uint256 netSyFee) = market.swapExactPtForSy(
            address(this),
            exactPTIn,
            swapData
        );
    ...
    function burnLP(
        ...
        (uint256 netSyOut, uint256 netPtOut) = market.burn(
            address(this),
            address(this),
            amount
        );
        if (netSyOut < minSyAmount) {
            revert InsufficientSyOut(netSyOut, minSyAmount);
        }
        if (netPtOut < minPtAmount) {
            revert InsufficientPtOut(netPtOut, minPtAmount);
        }
     ...
     * @param amount Amount by which the position is to be decreased
     * @param minAmount Minimum amount of base token redeemed
    ...
    function decreasePosition(
        ...
        // uint256 minAmount = SY.previewRedeem(_underlyingToken, _amount) * BASIC_POINT_DIVISOR / slippageTolerance ;
        IPStandardizedYield(address(SY)).redeem(
            address(this),
            amount,
            _underlyingToken,
            minAmount,
    ...
    function claimRewards(IPMarket market) external onlyManager nonReentrant whenNotPaused {
    ...
    function _getPositionTVL(
        ...
        if (positionInfo.positionTypeId == PENDLE_POSITION_ID) {
            ...
            (, address _underlyingToken, ) = _SY.assetInfo();
            {
                ...
                if (lpBalance > 0) {
                    underlyingBalance +=
                        ...
                        1e18; //(10 ** IERC20Metadata(_underlyingToken).decimals());
                }
            }
            {
            ...
            uint256 YTBalance = _YT.balanceOf(address(this));
            {
                ...
                if (YTBalance > 0 && !isExpired)
                    SYAmount += getYTValue(market, YTBalance);
            }
            if (SYAmount > 0)
                underlyingBalance += (SYAmount * _SY.exchangeRate()) / 1e18; //(10 ** IERC20Metadata(_underlyingToken).decimals());
    ...
    function getYTValue(
        ...
        (uint256 netSyOut, , , , , , ) = staticRouter.swapExactYtForSyStatic(
            market,
            balance
        );
        return netSyOut;
    ...
    /**
     * @notice Checks whether all positions (SY, PT, YT, and LP in a given market are empty for this contract
    ...
    function isMarketEmpty(IPMarket market) public view returns (bool) {
        ...
        return (_SY.balanceOf(address(this)) == 0 &&
            _PT.balanceOf(address(this)) == 0 &&
            _YT.balanceOf(address(this)) == 0 &&
