...
contract MorphoBlueConnector is BaseConnector {
     ...
     */
    function supply(uint256 amount, Id id, bool sOrC) external onlyManager nonReentrant whenNotPaused {
    ...
    function withdraw(uint256 amount, Id id, bool sOrC) external onlyManager nonReentrant whenNotPaused {
        MarketParams memory params = morphoBlue.idToMarketParams(id);
        if (sOrC) {
            (uint256 assets,) = morphoBlue.withdraw(params, amount, 0, address(this), address(this));
    ...
    function borrow(uint256 amount, Id id) external onlyManager nonReentrant whenNotPaused {
    ...
    function repay(uint256 amount, Id id) public onlyManager nonReentrant whenNotPaused {
        MarketParams memory params = morphoBlue.idToMarketParams(id);
    ...
    function repayWithCheck(uint256 amount, Id id) public onlyManager nonReentrant whenNotPaused {
        MarketParams memory params = morphoBlue.idToMarketParams(id);

        Position memory p = morphoBlue.position(id, address(this));
        Market memory market = morphoBlue.market(id);
        uint256 suppliedAmount = uint256(p.supplyShares).toAssetsUp(market.totalSupplyAssets, market.totalSupplyShares);
        amount = amount > suppliedAmount ? suppliedAmount : amount;
    ...
    function _getPositionTVL(HoldingPI memory p, address base) public view override returns (uint256 tvl) {
        ...
        if (positionInfo.positionTypeId == MORPHO_POSITION_ID) {
            ...
            // uint256 collateralTokenDecimals = IERC20Metadata(params.collateralToken).decimals();
            // uint256 loanTokenDecimals = IERC20Metadata(params.loanToken).decimals();

            tvl = _getValue(
                params.loanToken,
                base,
                (supplyAmount + convertCToL(pos.collateral, params.oracle, params.collateralToken)) - borrowAmount
            );
        }
    }
