...
contract BalancerFlashLoan is IFlashLoanRecipient, ReentrancyGuard {
    ...
    error InvalidConnector(uint256 vaultId, address connector);
    ...
    function receiveFlashLoan(
        ...
        if (!(caller == keeperContract) || caller == address(0)) {
        ...
        if (registry.isAnActiveConnector(vaultId, receiver)) {
            ...
            for (uint256 i = 0; i < destinationConnector.length; i++) {
                if (!registry.isAnActiveConnector(vaultId, destinationConnector[i])) {
                    revert InvalidConnector(vaultId, destinationConnector[i]);
                }
            ...
            revert InvalidConnector(vaultId, receiver);
        }
        for (uint256 i = 0; i < tokens.length; i++) {
