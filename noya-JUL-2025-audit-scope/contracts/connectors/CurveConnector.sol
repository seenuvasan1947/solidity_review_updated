...
contract CurveConnectorMainnet is BaseConnector {
    // ------------ state variables -------------- //
     ...
     */
    function depositIntoGauge(address pool, uint256 amount) public onlyManager nonReentrant whenNotPaused {
     ...
     */
    function depositIntoPrisma(address pool, uint256 amount, bool curveOrConvex) public onlyManager nonReentrant whenNotPaused {
        PoolInfo memory poolInfo = _getPoolInfo(pool);
     ...
     */
    function withdrawFromConvexBooster(uint256 pid, uint256 amount) public onlyManager  whenNotPaused{
     ...
     * @param amount - amount of tokens to withdraw
     */
    function withdrawFromConvexRewardPool(address pool, uint256 amount) public onlyManager whenNotPaused {
     ...
     * @param amount - amount of tokens to withdraw
     */
    function withdrawFromGauge(address pool, uint256 amount) public onlyManager whenNotPaused {
     ...
     * @param amount - amount of tokens to withdraw
     */
    function withdrawFromPrisma(address depostiToken, uint256 amount) public virtual onlyManager whenNotPaused {
     ...
     */
    function harvestRewards(address[] calldata gauges) public onlyManager nonReentrant whenNotPaused {
     ...
     */
    function harvestPrismaRewards(address[] calldata pools) public virtual onlyManager nonReentrant whenNotPaused {
     ...
     */
    function harvestConvexRewards(address[] calldata rewardsPools) public virtual onlyManager nonReentrant whenNotPaused {
        for (uint256 i = 0; i < rewardsPools.length; i++) {
            ...
            for (uint256 y = 0; y < extraRewardsLength; y++) {
                ...
                address extraRewardToken = IConvexBasicRewards(extraRewardsPool).rewardToken();
                _updateTokenInRegistry(extraRewardToken);
            }
        }
        _updateTokenInRegistry(CVX);
        _updateTokenInRegistry(CRV);
    ...
    function totalLpBalanceOf(PoolInfo memory info) public view returns (uint256) {
        ...
        uint256 convexLpBalance = balanceOfConvex(info);
        ...
        return lpBalance + convexLpBalance + rewardBalance + convexRewardBalance + prismaBalance + prismaConvexBalance;
    }
    ...
    /**
     * @notice Get total LP token balance of Convex pool
     * @param info - Curve pool info
     * @return totalLpBalance - total LP token balance of this contract in Convex pool
     */
    function balanceOfConvex(PoolInfo memory info) public view returns (uint256) {
        if (info.convexLPToken == address(0)) return 0;
        return IERC20(info.convexLPToken).balanceOf(address(this));
    }

    /**
     * @notice Withdraw tokens from Curve pool if isKilled
     * @param pool - Curve pool address
     * @param amount - amount of LP tokens to withdraw
     * @param minAmount - minimum amount of tokens to receive
     */
    function killedPoolLpWithdraw(address pool, uint256 amount, uint256 minAmount) public onlyManager nonReentrant {
        PoolInfo memory poolInfo = _getPoolInfo(pool);
        bytes32 positionId = registry.calculatePositionId(address(this), CURVE_LP_POSITION, abi.encode(pool));

        ICurveSwap(poolInfo.pool).remove_liquidity(amount, minAmount);

        uint256 tokenLength = poolInfo.tokens.length;
        for (uint256 y = 0; y < tokenLength; y++) {
            _updateTokenInRegistry(poolInfo.tokens[y]);
        }
        if (totalLpBalanceOf(poolInfo) == 0) {
            registry.updateHoldingPosition(vaultId, positionId, "", "", true);
        }
