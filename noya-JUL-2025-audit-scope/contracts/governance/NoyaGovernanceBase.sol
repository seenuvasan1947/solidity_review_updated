...
contract NoyaGovernanceBase {
    ...
    /**
     * @notice Reports that the vault is paused and cannot be interacted with
     */
    error NoyaGovernance_Paused();
    ...
    modifier onlyManagerOrWatcher() {
        (,,, address keeperContract,address watcherContract, address emergencyManager) = registry.getGovernanceAddresses(vaultId);
        if (!(msg.sender == keeperContract || msg.sender == emergencyManager || msg.sender == watcherContract)) {
            revert NoyaGovernance_Unauthorized(msg.sender);
        }
        _;
    }
    ...
    modifier whenNotPaused() {
        if (registry.isVaultPaused(vaultId)) revert  NoyaGovernance_Paused();
        _;
    }
}
