...
contract Game is Ownable {
    ...
    // 
    // q [fixed] is percentage check did for negative ?? like -1 etc range

    // q [fixed] Literal Instead of Constant
    ...
    // q [fixed] is nonReentrant is at last modifier is ok ??
    function claimThrone() external payable gameNotEnded nonReentrant {
        ...
        // q [fixed] is that if 1*1 is properlly handled
     ...
     */
    // q [fixed] is no one call what happen ??
    function declareWinner() external gameNotEnded {
        require(currentKing != address(0), "Game: No one has claimed the throne yet.");
        require(
            // what happens if any one call at that time if =  , same as expired time ??
            //  is can be explaited slighly using block.timestamp
